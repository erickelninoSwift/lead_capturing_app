...
const Modal = ({ buttonModal, mode, data, dataToupdate, setDatatoUpdate }) => {
  const currentData = mode ? dataToupdate : data;
  ...
  const handleAction = async (e) => {
    ...
    if (mode) {
      const response = await fetch(
        `http://localhost:8080/leads/${mode && currentData._id}`,
        {
          method: "PUT",
      ...
      if (datafetched.detail) {
        setCustomError(datafetched.detail);
        setTimeout(() => {
      ...
      <div className="w-full max-w-lg bg-white shadow-lg rounded-md p-6 relative">
        ...
        {!mode && (
          <>
            <div className="bg-blue-800 px-10 py-10 text-center text-white">
              ...
              <p className="text-center text-blue-100">contact details</p>
            </div>
            ...
            <div className="space-y-4 px-8 py-10">
              <label className="block" htmlFor="name">
                ...
                <input
                  className="w-full border-none rounded-md border bg-white px-2 py-2 outline-none ring-blue-600 focus:ring-1"
                  type="text"
              ...
              <label className="block" htmlFor="name">
                ...
                <input
                  className="w-full rounded-md border-none bg-white px-2 py-2 outline-none ring-blue-600 focus:ring-1"
                  type="email"
              ...
              <label className="block" htmlFor="name">
                ...
                <input
                  className="w-full rounded-md border border-none bg-white px-2 py-2 outline-none ring-blue-600 focus:ring-1"
                  type="text"
        ...
        <div className="border-t flex justify-end pt-6 space-x-4">
          ...
          <button
            ...
            onClick={mode ? (e) => handleAction(e) : () => buttonModal(false)}
          >
            {mode ? "Save" : "Ok"}
          </button>
        </div>
